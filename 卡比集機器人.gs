...
function handleEvent(event) {
  ...
  if (/^#(匯入預設客戶資訊|匯入客戶資訊|預設客戶資訊|匯入停車資訊|匯入停車)\s*$/.test(text)) { const r = seedParking(); replyToLine(replyToken, '🅿️ 已匯入預設客戶資訊 ' + r.count + ' 筆：\n' + r.keys.join('、') + '\n\n打「查鳳山雅君」或「客戶停車位置查詢」試試。'); return; }
  if (/^#清理訊息紀錄/.test(text)) {
  ...
  if (text === '#取消客戶') { removeGroupCustomer(chatId); replyToLine(replyToken, '已取消這個群組的客戶綁定。'); return; }
  ...
  if (text === '#清空冰庫名單' || text === '#清除冰庫名單') { setWarehouses([]); replyToLine(replyToken, '✅ 已清空冰庫名單。'); return; }
  if (/^#新增冰庫/m.test(text)) {
    const added = [];
  ...
  if (/^#新增貨主/.test(text)) { const n = text.replace(/^#新增貨主/, '').trim(); if (!n) { replyToLine(replyToken, '請加名稱，例：#新增貨主 旭陽'); return; } addVendor(n); replyToLine(replyToken, '✅ 已新增貨主：' + n + '\n目前名單：' + getVendors().join('、')); return; }
  if (/^#移除貨主/.test(text)) { const n = text.replace(/^#移除貨主/, '').trim(); removeVendor(n); replyToLine(replyToken, '已移除貨主：' + n + '\n目前名單：' + getVendors().join('、')); return; }
  ...
  if (text === '#安靜') { PROPS.setProperty('QUIET', '1'); replyToLine(replyToken, '🤫 已開啟安靜模式。'); return; }
  if (text === '#取消安靜') { PROPS.setProperty('QUIET', ''); replyToLine(replyToken, '🔊 已關閉安靜模式。'); return; }
  ...
  if (isShippingPullOutput(text)) {
    ...
    if (/清除/.test(text) && /確定/.test(text)) {
      if (!isAdmin(source.userId) && PROPS.getProperty('OWNER_USER_ID')) { replyToLine(replyToken, '🔒 清除紀錄僅限老闆使用。'); return; }
...

  {
    ...
    if (recs.length && !recs.some(function (r) { return /^(我|你|妳|他|她|牠|大家|誰|有人)$/.test(r.name); })) {
      recs.forEach(function (r) { recordLoan(r.name, r.type, r.amt, r.when); });
      const out = recs.map(function (r) { const q = loanQuery(r.name); const net = q.match(/未還：(-?\d+)/); return '💵 ' + (r.when ? r.when.slice(0, 10).replace(/^\d{4}\//, '') + ' ' : '') + r.name + ' ' + r.type + ' ' + r.amt + ' 元' + (net ? '（未還共 ' + net[1] + ' 元）' : ''); });
      replyToLine(replyToken, out.join('\n')); return;
    }
  }
  ...
  if (/^(中控總覽|中控總攬|今日總覽|今日總攬|總覽|總攬|中控)\s*$/.test(text)) {
    if (!isAdmin(source.userId) && PROPS.getProperty('OWNER_USER_ID')) { replyToLine(replyToken, '🔒 中控總覽僅限老闆使用。'); return; }
  ...
  if (/^#設定評比/.test(text)) { if (!isAdmin(source.userId)) { replyToLine(replyToken, '🔒 此功能僅限老闆。'); return; } replyToLine(replyToken, setEvalConfig(text)); return; }
  {
    ...
    if (evm && /評比/.test(text)) {
      if (!isAdmin(source.userId) && PROPS.getProperty('OWNER_USER_ID')) { replyToLine(replyToken, '🔒 綜合評比僅限老闆使用。'); return; }
      ...
      replyToLine(replyToken, evaluation(marg, empF, chatId)); return;
    }
  }
  {
    const sgm = text.match(/^(?:查\s*)?(\S*?)\s*出勤統計\s*(.*)$/);
    if (sgm && /出勤統計/.test(text)) {
      if (!isAdmin(source.userId) && PROPS.getProperty('OWNER_USER_ID')) { replyToLine(replyToken, '🔒 出勤統計僅限老闆使用。'); return; }
      ...
      replyToLine(replyToken, attendanceStats(marg, empF)); return;
    }
  }
  let aqm = text.match(/^(查遲到|查請假|查上班|查下班|查員工出勤|查出勤|出勤查詢|出勤紀錄)\s*(.*)$/);
  if (aqm) {
    if (!isAdmin(source.userId) && PROPS.getProperty('OWNER_USER_ID')) { replyToLine(replyToken, '🔒 出勤查詢僅限老闆使用。'); return; }
    let filter = '';
  ...
  if (/台子\s*[×xX*]\s*\d/.test(text) && (/收\s*\d/.test(text) || /取消/.test(text) || /修改\s*\d/.test(text))) {
    ...
    if (tr.count > 0) { if (!quiet()) replyToLine(replyToken, tr.reply); return; }
  }
  if (/【/.test(text) && /[×xX*]\s*\d/.test(text) && (/收/.test(text) || /取消/.test(text) || /修改/.test(text)) && !/台子\s*[×xX*]\s*\d/.test(text) && !/[：:]\s*\d/.test(text)) {
    ...
    if (rr.count > 0) { if (!quiet()) replyToLine(replyToken, rr.reply); return; }
  }
  if (/【/.test(text) && /[：:]\s*\d/.test(text) && (/取消/.test(text) || /清除/.test(text) || /(?:修改|改)\s*\d/.test(text) || /(?:改\s*(?:包裝|容器|成)?|改為|→|➜)\s*[（(]?\s*(?:台子|紙箱|圓籃|袋子|箱)/.test(text)) && !/\d+\s*件/.test(text) && !/[×xX*]\s*\d/.test(text) && !/收/.test(text)) {
    ...
    if (fc.count > 0) { if (!quiet()) replyToLine(replyToken, fc.reply); return; }
  }

  {
    ...
    if (cancelLine) {
      ...
      if (isAttend || isFinance || isShipping) {
        if (!isAdmin(source.userId) && PROPS.getProperty('OWNER_USER_ID')) { replyToLine(replyToken, '🔒 取消紀錄僅限老闆使用。'); return; }
        if (isAttend) {
  ...
  if (!/^查/.test(text)) {
    ...
    if (action && emp) {
      ...
      if (!quiet()) replyToLine(replyToken, msg + '\n時間：' + nowStr());
      return;
    }
  }
  ...
  /* ---- 場外一行寫入冰庫：寄冰 客戶 品名[等級] 數量／+N／-N／修改N／改N／取消／容器 ---- */
  {
    ...
    if (/^寄冰\s+\S/.test(fl) && hasOp) {
      ...
      if (fcmd.count > 0) { if (!quiet()) replyToLine(replyToken, fcmd.reply); return; }
    }
  }
  ...
  if (/[(（]/.test(text)) {
    ...
    if (slip.length > 0) { if (!quiet()) replyToLine(replyToken, slip.join('\n\n')); return; }
  }

  if (/入庫|出庫/.test(text)) {
    const fb = handleFreezerBatch(text);
    if (fb.count > 0) { if (!quiet()) replyToLine(replyToken, fb.reply); return; }
  }

  if (!/【/.test(text) && /台子/.test(text) && /收/.test(text)) {
    const tc = handleTaiziCollectInline(text);
    if (tc.count > 0) { if (!quiet()) replyToLine(replyToken, tc.reply); return; }
  }

  if (/(?:扣除|損耗)\s*\d+\s*件|改\s*\d+\s*元|價格修正/.test(text)) {
    const slip = scanSlip(text, chatId);
    if (slip.count > 0) {
      if (!quiet()) replyToLine(replyToken, slipSummary(slip));
      notifyOwner('📩【記錄】\n' + slipSummary(slip));
      return;
    }
  }
  ...
  /* ---- 鐵架固定格式驗證（非【】單）：格式/名稱不對直接跳警示，正確才寫入 ---- */
  {
    ...
    if (rs.handled) { if (!quiet()) replyToLine(replyToken, rs.reply); return; }
  }
  ...
  if (/^(出去|出貨|出|回收|收)/.test(text.split('\n')[0].trim())) {
    ...
    if (rb.count > 0) { if (!quiet()) replyToLine(replyToken, rb.reply); return; }
  }

  if (/[*＊×xX]\s*\d+/.test(text) && !/【/.test(text)) {
    const ri = handleRackInlineOut(text);
    if (ri.count > 0) { if (!quiet()) replyToLine(replyToken, ri.reply); return; }
  }
...
/* ========================== 【工作群組/權限/別名 等基礎】 ========================== */
function quiet() { return PROPS.getProperty('QUIET') === '1'; }
...
function handleDuty(text) {
  ...
  const location = rest.replace(/出發|出門/g, '').replace(/^[:：]/, '').trim();
...
function commandSheet() {
  return [
    ...
    '【設定】', '・#版本　#設定客戶 名稱　#查客戶', '・#貨主名單／#新增貨主 X', '・#冰庫名單／#新增冰庫 X', '・#安靜／#取消安靜', '',
...
function isWholeIce(text) {
  ...
  return String(text).split('\n').some(function (l) { return /^[\s\-—─–_=]*冰[\s\-—─–_=]*$/.test(l); });
}
...
function evaluation(monthArg, empFilter, groupId) {
  ...
  const lines = scored.map(function (r, idx) { const x = r.x; return (idx + 1) + '. 【' + r.e + '】評分 ' + Math.round(r.score) + '\n　出勤 ' + r.d + ' 天（上' + x.on + '/下' + x.off + '）遲到 ' + x.late + ' 請假 ' + x.leave + (x.offLate > 0 ? '\n　晚下班加時 ' + (Math.round(x.offLate * 10) / 10) + ' 小時（+' + Math.round(x.offLate * cf.offlate) + '）' : '') + '\n　外勤 ' + x.dutyCnt + ' 次・加給 ' + x.dutyAmt + ' 元\n　工作留言 ' + x.work + ' ／廢話 ' + x.chatter; });
  return '🏆 ' + ym + ' 綜合評比（結算獎金用）：\n' + lines.join('\n――――\n') + '\n――――――\n權重：出勤×' + cf.day + ' 外勤×' + cf.duty + ' 工作×' + cf.work + ' 廢話×' + cf.chatter + ' 遲到×' + cf.late + ' 請假×' + cf.leave + ' 晚下班×' + cf.offlate + '(基準' + cf.offbase + '點)';
}
...
function attendanceStats(monthArg, empFilter) {
  const ym = resolveYM(monthArg) || thisYM();
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (act === '上班') { stat[emp].on++; days[emp][ymdStr(data[i][0])] = 1; }
    else if (act === '下班') { stat[emp].off++; days[emp][ymdStr(data[i][0])] = 1; }
    else if (act === '遲到') stat[emp].late++;
    else if (act === '請假') stat[emp].leave++;
    if (st === '遲到' && act !== '遲到') stat[emp].late++;
  }
  if (!order.length) return '🕒 ' + ym + (empFilter ? ' ' + empFilter : '') + ' 沒有出勤紀錄。';
  const filtered = order.filter(function (e) { return !isResigned(e); });
  if (!filtered.length) return '🕒 ' + ym + (empFilter ? ' ' + empFilter : '') + ' 沒有出勤紀錄。';
  filtered.sort(function (a, b) { return Object.keys(days[b]).length - Object.keys(days[a]).length; });
  const lines = filtered.map(function (e) {
    const s = stat[e]; const d = Object.keys(days[e]).length;
    return '【' + e + '】\n　出勤 ' + d + ' 天｜上班 ' + s.on + '｜下班 ' + s.off + '\n　遲到 ' + s.late + ' 次｜請假 ' + s.leave + ' 次';
  });
  return '🕒 ' + ym + ' 出勤統計（結算獎金用）：\n' + lines.join('\n') + '\n――――――\n（查單一員工：查X出勤統計　指定月：出勤統計 5月）';
}
