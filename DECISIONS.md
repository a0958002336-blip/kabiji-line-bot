...
## v3.3 輸入失敗追蹤（D-V33，2026/07/12）
- **新功能，不動既有判定/寫入邏輯**。新表 `SHEET_FAIL='輸入失敗紀錄'`（欄位：時間/類型/輸入者/原文/群組ID）。
- **D-V33.1 留檔點**：`logInputFail(type, source, chatId, text)` 於各「格式錯誤警示」回覆點呼叫——鐵架格式(`rackEntryGuard` warn／`rackSlipStrict` 加 `warn:true` 旗標後於呼叫點記)、收款無金額(recvDetect needAmount)、寄運無法判斷(ship.unresolved)、寄運無法確認(intentAllowsWrite fail)、冰庫貼回格式、寄運貼回格式。輸入者用 `getDisplayName(chatId,userId)`。**安靜模式被靜默的也照樣寫入**（log 在 quiet 判斷之外）。
- **D-V33.2 防灌表**：同一人同一分鐘（nowStr 分鐘精度）同原文只記一筆（回看最近 80 筆比對）；表超過 3000 筆自動 `deleteRows` 清最舊 500。
- **D-V33.3 查詢（ownerOnly 限老闆）**：`查輸入失敗`（全部，最近 50＋每人次數排行）／`今日輸入失敗`／`查輸入失敗 7/10`／`查輸入失敗 7/1-7/10`（區間）。每筆格式「時間｜輸入者｜類型｜原文前30字」，結尾附失敗次數排行（依次數排序）。指令置於權限閘門之前（老闆任何群可查）。
- **相容**：既有測試的 ERP 寫入計數清單未含「輸入失敗紀錄」，故格式錯誤訊息額外留檔不影響既有「零 ERP 寫入」斷言。新增 `run_v33.js`（16 案）。全量 395/395 綠。

## v3.4 去貨主名全位置＋#備註規則＋收款流程（D-V34，2026/07/14，使用者逐項確認）
- **D-V34.1 去貨主名(Bug1，A+B1)**：新增 `stripVendors(str)` 把「已登記貨主」名字從標題/明細/備註任何位置移除(含黏字)，套 `shippingCleanSummary`(客戶/品名/備註)＋寫入端品名/備註欄(貨主欄保留)。根因：貨主與品名黏字且未登記時 parseShipping 無法切開。**未登記貨主無法自動辨識→需 `#新增貨主` 補登**(使用者接受此工作流)。
- **D-V34.2 #備註規則(Bug2)**：① `#`開頭且非任何已知指令→`appendRecentNote` 掛到「今日最近一筆」寄運或收款的備註欄(兩者取時間較新；限當日；無資料回「今日尚無可掛備註的資料」)，放 handleEvent 末端當 fallback。② parseShipping 尾行**只吃括號備註**(如（修清）)，純文字閒聊不再自動當備註→修「可以連鐵架一起需紀錄」被誤存。③ 無`#`一般訊息維持既有：命中功能觸發照舊、否則靜默無視。**誤存那筆在正式試算表，需使用者於 GAS 手動清該格(本地無正式資料)**。
- **D-V34.3 收款流程(共用待收款表)**：待收款表加 3 欄(18收款人ID/19完款時間/20綁定訊息ID)，狀態沿用「未收/已收/取消」不新增列舉(收款中＝未收且收款人已填；已完款＝已收+完款時間)。指令：`#<客戶號>收款 名字`(recvOpenRows；同客戶多張未完款→列單號讓選、僅一張直接動)、`#<客戶號>[誰收款]`(狀態)、`#未收`(未完款清單+掛幾天)、`#已收 6986`(完款：保留已指定收款人、記完款時間col19，不覆蓋收款人)。既有 R編號 流程(收款人 Rxxx/#已收 R/#待收款)全相容。
- **D-V34.4 回覆綁定＋引用例外(使用者(c)：範圍寫死)**：`replyToLine` 解析 LINE 回應 `sentMessages[].id` 回傳；建立待收款時把 bot 訊息ID寫 col20。收到帶 `quotedMessageId` 的訊息→`receivableBindByQuote` 反查 col20：無收款人→綁定回覆者(LINE名+ID)；已他人→提示「回確認改人」；含「確認」→改人；同人/已結案→對應提示。此為 isQuote 唯讀 P0 的**唯一寫入例外**，寫死只允許「引用待收款→寫收款人」，其他引用維持唯讀。
- **測試**：新增 `run_v34.js`(32 案，Bug1/#備註/無#閒聊/收款指令/多張/回覆綁定/引用例外)；`run_receivable` 46 案不受影響(完款改為保留收款人，屬新流程正確化)；`run_version` v3.3→v3.4。全量 427/427 綠。

## v3.4.1 鐵架單行出庫誤攔修正（D-V341，2026/07/14，部署前補）
- **Bug**：真實人話「碼頭 美婷要一件田228 1*1200」被 handleRackInlineOut 誤判為出鐵架（名稱不含「鐵」也照收）。
- **修正**：鐵架單行出庫入口(handleEvent `/[*×]\d+/ && !【`)加 `&& /鐵/.test(text)`——需含「鐵」才判定為鐵架。所有正規鐵架名皆含「鐵」(旭陽鐵架/勝山鐵架…)，故正常出鐵架不受影響；順帶收斂 C1(台子*N inline 不再誤記鐵架，改靜默)。
- **測試**：`run_v34` ③3 新增「碼頭 美婷要一件田228 1*1200」與「可以連鐵架一起需紀錄」並列為「閒聊裝死」防迴歸(無回應/零寫入含鐵架)。全量 428/428 綠。
