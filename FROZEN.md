...
本 repo 為卡比集 LINE 機器人，目前 **v3.3 已就緒待部署**。

**未經明確指示禁止任何修改。**

---

- 待部署版本：`develop`（deploy commit `65effd4`；`#版本` 顯示 `88b3d31`；`v3.3`）
- 封版日期：2026/07/12（v3.2 計價單日期戳/代號收回；v3.2.1~3 外勤/日期戳唯讀群/件數查詢防呆；v3.3 輸入失敗追蹤）
- 回歸測試：`node tests/golden/run_all.js` → 395/395 全綠
