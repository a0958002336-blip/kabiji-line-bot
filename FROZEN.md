...
本 repo 為卡比集 LINE 機器人，目前 **v3.2.2 已就緒待部署**。

**未經明確指示禁止任何修改。**

---

- 待部署版本：`develop`（deploy commit `dc7b13d`；`#版本` 顯示 `d92b450`；`v3.2.2`）
- 封版日期：2026/07/10（v3.2：計價單日期戳＋代號前綴收回＋名稱式收回擋代號；v3.2.1：外勤回覆精簡；v3.2.2：修日期戳唯讀群不觸發）
- 回歸測試：`node tests/golden/run_all.js` → 369/369 全綠
