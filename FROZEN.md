...
本 repo 為卡比集 LINE 機器人，目前 **v3.2.1 已就緒待部署**。

**未經明確指示禁止任何修改。**

---

- 待部署版本：`develop`（deploy commit `84bfd46`；`#版本` 顯示 `00f4755`；`v3.2.1`）
- 封版日期：2026/07/09（v3.2：計價單日期戳＋代號前綴收回＋名稱式收回擋代號；v3.2.1：外勤即時回覆精簡＋查詢累計標月份）
- 回歸測試：`node tests/golden/run_all.js` → 365/365 全綠
