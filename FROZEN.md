...
本 repo 為卡比集 LINE 機器人，目前 **v3.4.1 已就緒待部署**。

**未經明確指示禁止任何修改。**

---

- 待部署版本：`develop`（deploy commit `f27cb24`；`#版本` 顯示 `881e268`；`v3.4.1`）
- 封版日期：2026/07/14（v3.4 去貨主名全位置＋#備註規則＋收款流程；v3.4.1 鐵架單行出庫需含「鐵」修人話誤攔）
- 回歸測試：`node tests/golden/run_all.js` → 428/428 全綠
