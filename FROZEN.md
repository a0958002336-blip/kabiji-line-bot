...
本 repo 為卡比集 LINE 機器人，目前 **v3.4.8 已就緒待部署**（含 v3.4.7，兩版一起上）。
...
>
> 本版即為 **`f638800`**（`[v3.4.8] …` 那個 commit），**不是** `91d8b1c`（chore 回填）、
> 也不是封版 commit。流程固定：① 版本 commit（BOT_BUILD 仍是上一版）→ ② 取該 commit 短 hash
> → ③ 回填並另開 `chore(版本)` commit → ④ 封版 commit。故 BOT_BUILD 會比 HEAD 少一到兩格，
> **這是刻意的、不是漏更新**。做 verify_sync 方法 A 時請拿 `f638800` 比對，不要拿 `git rev-parse HEAD`。
> ❌ 禁止用 `git commit --amend` 回填（amend 改掉 hash → 填進去的值當場失效）。

---

- 待部署版本：`develop`（deploy commit `91d8b1c`；`#版本` 顯示 `f638800`；`v3.4.8`）
- 封版日期：2026/08/11（v3.4.8 新增 `#備份狀態`；含未部署的 v3.4.7 查出勤月份篩選）
- 回歸測試：`node tests/golden/run_all.js` → 552/552 全綠
- ⭐ **部署後第一件事**：LINE 打 `#備份狀態` → 若顯示「❌ 未建立」，
  到 GAS 執行 `setupBackupTrigger` 一次，再打一次確認變 ✅。
  這支指令就是為了取代「每次貼腳本進 GAS 查」而做的。
- 🚫 **v3.4.7 / v3.4.8 皆尚未部署**（貼一次 v3.4.8 即兩版一起上；push 與部署待老闆指示）
...
- 桌面部署檔（三版並存，舊版保留作回退點，勿刪）：
  - **本次** `0_最新程式_貼這個_v3.4.8.txt` ／ `卡比集機器人-v3.4.8.gs`（sha256 `ffff0913…`，對齊 HEAD）
  - v3.4.7（未部署）`0_最新程式_貼這個_v3.4.7.txt` ／ `卡比集機器人-v3.4.7.gs`（sha256 `ffdcadf5…`）
  - v3.4.6（線上現行）`0_最新程式_貼這個_v3.4.6.txt` ／ `卡比集機器人-v3.4.6.gs`（sha256 `fb667cf3…`）
  - v3.4.5（回退點）`0_最新程式_貼這個_v3.4.5.txt` ／ `卡比集機器人-v3.4.5.gs`（sha256 `1da17aa2…`）
  - 另有 `diag_carrier.gs`（主車行核對唯讀診斷，非部署檔；貼 GAS 新檔案跑完即刪）
- ⚠️ 每日備份仍需部署後手動啟用一次：GAS 執行 `setupBackupTrigger`（見 `docs/備份與還原手冊.md` 第五節）。
  **v3.4.8 起可直接在 LINE 打 `#備份狀態` 確認有沒有建**（不必再貼腳本進 GAS）。
