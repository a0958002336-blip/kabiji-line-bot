...
本 repo 為卡比集 LINE 機器人，目前 **v3.4.2 已就緒待部署**。

**未經明確指示禁止任何修改。**

---

- 待部署版本：`develop`（deploy commit `8381277`；`#版本` 顯示 `fbee1c7`；`v3.4.2`）
- 封版日期：2026/07/15（v3.4.2 鐵架收緊A方案：全路徑單行/多行/首行動詞/前置閘一律需含「鐵架」二字才觸發，修多行交易人話「進口山東 *5」被誤攔）
- 回歸測試：`node tests/golden/run_all.js` → 431/431 全綠
- ⚠️ 部署後：新貨主去名沒去到時 `#新增貨主 X` 補登；「可以連鐵架一起需紀錄」誤存那格請於 GAS 手動清（本地無正式資料）
- 桌面部署檔：`C:\Users\sqluser\Desktop\卡比集機器人-v3.4.2.gs`（本次；sha256 已對齊 HEAD `4ba87af…`）
