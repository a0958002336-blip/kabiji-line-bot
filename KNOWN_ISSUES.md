...
### Task 4 收款模組 — ✅ 已完成（rc-2，決策 D1 後）
- 已於 `卡比集機器人_repo` 全新建「待收款」18欄分頁 + recvDetect/建立/查詢/#已收/#取消收款(軟刪除+Audit)。26 測試通過。詳見 DECISIONS D4.*。
- 註：本模組與 kabiji-bot 的收款模組為**各自獨立實作**（schema 一致、程式碼未互搬）；兩 repo 仍不同步，正式來源＝卡比集機器人_repo（使用者已確認）。
